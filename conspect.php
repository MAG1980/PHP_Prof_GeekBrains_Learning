...
При этом автоматически происходит остановка работы скрипта, но фатальной ошибки,
которую мог в этом случае сгенерировать PHP-интерпретатор, не возникает.
Поэтому остаётся возможность обработать данное исключение, например, сделать перенаправление.
...
т.е. весь код проекта достаточно обернуть в один блок try{} catch {$exception} (в точке входа в приложение).
...
"autoload": {
		"psr-4": {
			"app\\": "echo " - где app - префикс классов проекта, а в кавычках - путь к папке с классами относительно корневой папки проекта (совпал с корнем проекта)
											}
										},
									...
									Чтобы провайдер данных применился, необходимо указать его в теге @dataProvider к тесту.
									...
									В настоящее время SingleTone  считается антипаттерном. Им можно пользоваться только на вершине цепочки наследования, т.е.,
									если не требуется создвать наследников.
									
									Класс app инкапсулирует всю работу приложения в один класс.
									В нём будут создаваться и храниться экземпляры остальных классов.
									
									Константы удобно хранить в массиве. Будем возвращать массив констант из файла config.php с помощью return;
									
									Сделаем App синглтоном с помощью трейта;
									
									Массив с константами будем подключать в точке входа с помощью include и передавать в метод run($config) класса App.
									
									Автозагрузчики подключаем в точке входа (index.php).
									
									Запуск приложения: 
									
									App::getInstance()->run($config);
									
									Будем использовать паттерн Хранилище (Storage): компоненты можно хранить в ассоциативном массиве, который хранится в отдельном объекте.
									
									Согласно паттерну Хранилице у объекта хранилица должно быть только два метода: set() и get().
									
									Принципы SOLID позволяют создавать легко расширяемые приложения. Принципы SOLID являются рекомендацией.
									 От принципов SOLID можно отступать только в том случае,
									если расширять приложение точно не придётся.
									
									Переименовать метод можно с помощью метода, возвращающего исходный метод.
									
									Чтобы ограничить варианты создаваемых сущностей, будем хранить в файле config.php
									поле 'components'=>['имя компонента'=>['class'=>Db::class],...] - Db::class - полное имя класса вместе с namespace. 
									...
									/**
									* Class App
									* @property Request - будет подсказывать Storm, что в объекте есть поле Request.
									*/
